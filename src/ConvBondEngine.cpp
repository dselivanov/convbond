...
{	
	...
	{
		nbOfDividends = rngDividendSchedule[0].size();
	}
	...
	double u = exp(sigma * sqrt( dt )), d =1 / u;
	double p = (exp( ( intRate ) * dt) - d) / (u - d);
	int i, j, k;
	//column0=amounts column2=dates
	/*double **inputDividendSchedule=new double*[2];
	for (i=0;i<2;i++)
	{
		inputDividendSchedule[i] = new double[nbOfDividends];
	}
	// initializing dividend schedule
	for (i=0;i<nbOfDividends;i++)
	{
		//inputDividendSchedule[DIVIDEND_AMOUNT_DIMENSION][i] = rngDividendSchedule(i, DIVIDEND_AMOUNT_DIMENSION);
		//inputDividendSchedule[DIVIDEND_DATE_DIMENSION][i] = rngDividendSchedule(i, DIVIDEND_DATE_DIMENSION);
		inputDividendSchedule[DIVIDEND_AMOUNT_DIMENSION][i] = rngDividendSchedule[i][DIVIDEND_AMOUNT_DIMENSION];
		inputDividendSchedule[DIVIDEND_DATE_DIMENSION][i] = rngDividendSchedule[i][DIVIDEND_DATE_DIMENSION];
	}*/
	...
	{
		i = 0;
		//while ((dt * (i+1) < inputDividendSchedule[DIVIDEND_DATE_DIMENSION][j]) && (i < (nbTreeLevels-1)))
	...
	dividendAdj = prod(dividendSchedule, createIndex(nbTreeLevels), nbTreeLevels);
	conversionAdj = 1 / prod_with_cond(dividendSchedule, createIndex(nbTreeLevels), nbTreeLevels, dividendProtectionRate);
	for( i = 0; i < nbTreeLevels; i++)
	{
    ...
    for(k = nbTreeLevels - 2; k>=0; k--)
	{
		dividendAdj = prod(dividendSchedule, createIndex(k), k);
		conversionAdj = 1 / prod_with_cond(dividendSchedule, createIndex(k), k, dividendProtectionRate);
		i=0;
		for(i = 0; i <=k; i++)
		{
            ...
            //forcing conversion
            if ((barrier > 0) && (treeS[i][k] >= barrier) && (barrierStartTime <= k * dt))
			{
                treeConversionProb[i][k] = 1;
                conversionValue = conversionAdj * conversionRatio * barrier / faceValue;
                treeDiscRate[i][k] = intRate * treeConversionProb[i][k] + 
											intRateRisky * (1 - treeConversionProb[i][k]);
                discFactorRiskyUp = exp(-treeDiscRate[i + 1][k + 1] * dt);
                discFactorRiskyDown = exp(-treeDiscRate[i][k + 1] * dt);
                treeConversionValue[i][k] = couponSchedule[k]
													+ p * discFactorRiskyUp * treeConversionValue[i + 1][k + 1]
													+ (1 - p) * discFactorRiskyDown * treeConversionValue[i][k + 1];
                treeConversionValue[i][k] = max(min(treeConversionValue[i][k], barrierStrike), conversionValue);
            }
	...
	double nbStepsPerYear)
{
	...
	ptr_convBondParamStr-> barrierStartTime=barrierStartTime;
	ptr_convBondParamStr-> barrier=barrier;
	ptr_convBondParamStr-> barrierStrike=barrierStrike;
...
double CBprice_wrapper_coupon(double couponRate, convBondParam *ptr_convBondParamStr)
{
  double price=CBprice(
	...
	ptr_convBondParamStr-> barrierStartTime,
	ptr_convBondParamStr->barrier,
	ptr_convBondParamStr->barrierStrike,
	...
	double nbStepsPerYear)
{
	...
	ptr_cbparam1=init_convBondParam(S_0, sigma, intRate0, creditSpread, rateCompounding, 
					...
					faceValue, redemtionPremium, barrierStartTime, barrier, barrierStrike, putStartTime, putStrike,
